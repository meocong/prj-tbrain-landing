...
| ~~Hero copy~~ | **written.** "See the data before you buy it. / Robotics, gaming, coding and STEM." Deliberately at the level of the whole library, not one line of it: the first attempt read "Data for models that have to act / Human hands, robot arms, live play", which is a robotics headline on a page that also sells gaming and Coding & STEM. Every tier, hour count and price differs per category and is stated on the category, so the subhead ends by pointing there. Tam can swap the words; leaving the old line up while waiting was the wrong call. | R19 |
