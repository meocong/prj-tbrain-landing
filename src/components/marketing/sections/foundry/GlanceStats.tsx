...
// Token-based accents adapt light/dark automatically (WCAG AA on paper).
const STATS: { k: string; v: number; suffix?: string; sub: string; color: string }[] = [
  { k: "Models · auto-label pipeline",  v: 8,   sub: "Hand · body · masks · depth · verb-noun", color: "var(--bp-cyan)" },
  { k: "Hard rules · per capture",       v: 15,  sub: "Machine-checkable QC gate",              color: "var(--bp-cyan-strong)" },
  { k: "Ship rate · after QC",           v: 92,  suffix: "%", sub: "Auto-accept + Label Studio + reviewer", color: "var(--bp-purple)" },
  { k: "Delivery latency",               v: 48,  suffix: "h", sub: "Raw → LeRobot v2 · zero-trust", color: "var(--bp-amber)" },
];

export function GlanceStats() {
  return (
    <section className="relative overflow-hidden" style={{ padding: "clamp(48px,6vw,80px) 0", color: "var(--bp-ink)" }}>
