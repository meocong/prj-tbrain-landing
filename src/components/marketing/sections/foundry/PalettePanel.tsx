 ...
 * introduced in the burn-pipeline hardening pass (git 1b0cce1).
...
const PALETTE = [
  ...
  { source: "sam3",          label: "SAM3 (mask only)",    color: "#f0a2ff", note: "Object segmenter mask · hand-tracking suppressed" },
  { source: "sapiens_wrist", label: "Sapiens wrist",       color: "#ff9a4d", note: "Wrist fallback where HaWoR fails · gated" },
  { source: "moge",          label: "MoGe depth",          color: "#00e5c7", note: "Metric depth + pointmap · K_hawor priority" },
  { source: "human",         label: "human correction",    color: "#5ee08a", note: "Label Studio diff · manifest override" },
];

const WATERMARKS = [
  { key: "NO_DET",       color: "#ff5f57", note: "Hand missing this frame · flagged" },
  { key: "CLOSE_HAND",   color: "#ff9a4d", note: "Skin-close-to-hand · HITL priority" },
  { key: "MESH_OOB",     color: "#ff5f57", note: "Mesh projected outside frame · dropped" },
  { key: "MESH_DISPUTE", color: "#ff9a4d", note: "Mesh vs. hand-track disagree · gate" },
  { key: "MASK_DRIFT",   color: "#ff9a4d", note: "Mask/bbox ratio > 1.5× · skipped from ship" },
  { key: "BODY_HIDDEN",  color: "#8fa0c8", note: "Dense body off default · off-frame body suppressed" },
];
  ...
  return (
    <Sheet id="palette" fig="FIG.05H — PROVENANCE PALETTE · BURN v1b0cce1" axis>
      <SheetHeading
        ...
        lead="After the pipeline hardening pass, the annotated.mp4 encodes provenance and failure modes in the visual itself. Buyer never has to grep the manifest to know which model produced which pixel."
      />
      ...
      <div className="mt-6 bp-mono" style={{ fontSize: 11, color: "var(--bp-ink-dim)", padding: "12px 14px", background: "color-mix(in srgb, var(--bp-cyan) 4%, transparent)", borderRadius: 10, lineHeight: 1.6 }}>
        ...
        Every colored dot / mesh in <code style={{ color: "var(--bp-cyan)" }}>labels/annotated.mp4</code> maps 1:1 to a source in the manifest. Every watermark maps to a flag. LeRobot exports strip debug frames; HITL queue reads CLOSE_HAND priority. All 20 caps rebuilt + reburned at git <code style={{ color: "#00e5c7" }}>1b0cce1</code> · 152/152 tests PASS.
      </div>
    </Sheet>
  );
}
