...
/**
 * Passcode entry for the sample library.
 *
...
const PROJECT = "samples";
const DEFAULT_BATCH = "library";
...
export async function POST(req: NextRequest) {
  ...
  const batchSlug = body.batchSlug?.trim() || DEFAULT_BATCH;
  ...
  for (const p of candidates ?? []) {
    ...
    const kind = p.client_id != null ? "grant" : "batch_passcode";
    ...
    const { error: bumpErr } = await db
      ...
      .eq("id", p.id);
    if (bumpErr) {
      console.error("[samples] passcode use_count update failed:", bumpErr);
    }
    ...
    await Promise.all([
      logAuthAttempt({ ip, batchId: batch.id, success: true, meta: { kind, project: PROJECT } }),
      logEvent({
  ...
  await Promise.all([
    logAuthAttempt({ ip, batchId: batch.id, success: false, meta: { prefix, project: PROJECT } }),
    logEvent({ batchId: batch.id, eventType: "enter_fail", ip, userAgent, meta: { prefix, project: PROJECT } }),
  ]);
