...
import { getCaseStudyBlocks, type CaseStudyBlock } from "@/lib/landing/case-study-blocks";
...
function CaseStudyWidgetRenderer({
  blocks,
  fallbackMetrics,
}: {
  ...
  return (
    <>
      {blocks.map((block) => {
        switch (block.type) {
          case "metrics_grid":
            return <MetricsGrid key={block.id} metrics={asMetrics(block.config.metrics) || fallbackMetrics} />;
          case "text_card":
            return <TextCard key={block.id} block={block} />;
          case "objective_grid":
            return <ObjectiveGrid key={block.id} block={block} />;
          case "challenge_cards":
            return <ChallengeCards key={block.id} block={block} />;
          case "qa_framework":
            return <QaFramework key={block.id} block={block} />;
          case "process_steps":
            return <ProcessSteps key={block.id} block={block} />;
          case "outcome":
            return <OutcomeBlock key={block.id} block={block} />;
          case "cta":
            return <LegacyCta key={block.id} title={block.title || "Need Expert Data Services?"} subtitle={block.subtitle || ""} config={block.config} />;
          default:
            return null;
        }
      })}
...
function asMetrics(value: unknown): Metric[] | null {
