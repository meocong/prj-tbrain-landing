...
export function ContentClient() {
  ...
  const [tagFilter, setTagFilter] = useState<string | null>(
    typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("tag") : null
  );
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-posts", search, statusFilter, tagFilter, page],
    queryFn: async () => {
      ...
      <div className="glass-card mt-4 flex flex-wrap items-center gap-3 p-3">
        ...
        {tagFilter && (
          ...
          >
            ...
            {tagFilter}
            <button onClick={() => { setTagFilter(null); setPage(0); }} aria-label="Clear tag filter">
              <X className="h-3 w-3" />
            </button>
          </span>
        )}
