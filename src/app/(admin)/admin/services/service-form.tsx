...
const ICON_OPTIONS = [
  "Brain", "Tags", "BarChart3", "Database", "ShieldCheck", "Users", "Bot",
] as const;
...
export function ServiceForm({ initial }: { initial?: ServiceFormValues }) {
  ...
  const [form, setForm] = useState<ServiceFormValues>(initial ?? EMPTY);
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (id) => {
      ...
      if (!isEdit) router.push(`/admin/services/${id}`);
      else router.refresh();
    },
  ...
  return (
    <div className="animate-[fadeIn_0.3s_ease-out] max-w-2xl">
      <Link href="/admin/services" className="inline-flex items-center gap-1 text-xs mb-3" style={{ color: "var(--text-muted)" }}>
      ...
      <div className="space-y-5 rounded-2xl p-6" style={{ background: "var(--bg-elevated, #fff)", border: "1px solid var(--border-default)" }}>
        ...
        <Field label="Description">
          <textarea
            ...
            placeholder="Multi-modal annotation across text, image, video, and audio."
          />
        </Field>
        ...
        <div className="flex items-center justify-between pt-2">
          {isEdit ? (
            <button
              type="button"
              onClick={() => { if (confirm("Delete this service?")) deleteMutation.mutate(); }}
