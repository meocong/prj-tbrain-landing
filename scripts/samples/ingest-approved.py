...
def ego_record(folder, slugs, dry, report):
    ...
    spec = [
        ...
        ["Cameras", "Six on the head in three stereo pairs · primary and mid pairs delivered"],
    ...
    return {
        ...
        "preview": f"Preview · {span:.0f} s on the four delivered lenses, the same instant on each · {W} x {H} per view",
        "pills": [
            {"t": task.get("skill_group"), "k": "skill"},
            {"t": task.get("task_difficulty"), "k": "difficulty"},
            {"t": "6-camera rig", "k": "quality"},
