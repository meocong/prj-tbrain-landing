 ...
 * Never on a dry run.
 *
 * `--dry` skips the upload and used to write the manifest anyway, which breaks
 * the one invariant this file exists to hold: the manifest is generated by the
 * same pass that puts the objects in the bucket, so it cannot claim an asset
 * the bucket does not have (see the note on `downloads.ts`). A dry run left
 * `downloads.json` describing 193 objects where 131 existed, and the download
 * route would then have signed a URL for each of the 62 that did not — a signed
 * 404, which reads to a customer as a broken download rather than as a file
 * nobody has uploaded yet. That is exactly the case `not_staged` exists to
 * avoid, routed around by the tool that is supposed to maintain it.
 *
 * Caught by running `--dry` to preview this change and finding the working tree
 * dirty afterwards.
 */
if (!ONLY && !DRY) {
  ...
  console.log(`\ndry run — manifest NOT written (${Object.keys(manifest.samples).length} samples would be staged)`);
}
